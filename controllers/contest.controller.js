...
export const luckyDraw = async (req, res, next) => {
    try {
        ...
        // let ContestObj = await Contest.findOne({ _id: req.params.id, status: "APPROVED" }).exec();
        // if (!ContestObj) throw { status: 400, message: "Contest Not Found" };
        // await Contest.findByIdAndUpdate(req.params.id, { status: "CLOSED" }).exec();

        // let contestPrizes = await Prize.find({ contestId: req.params.id }).sort({ rank: 1 }).lean().exec();
        // let contestUsers = await userContest.find({ contestId: req.params.id }).lean().exec();
        ...
        //         var randomItem = contestUsers[Math.floor(Math.random() * contestUsers.length)];
        //         await userContest.findByIdAndUpdate(randomItem._id, { status: "win", rank: prize?.rank }).exec();
        //         contestUsers = contestUsers.filter(el => `${el._id}` != `${alreadyWinner._id}`);

        //     }
        // }
        // await userContest.updateMany({ contestId: req.params.id, status: "join" }, { status: "lose" }).exec();
...
export const previousContest = async (req, res, next) => {
    try {
        ...
        let ContestObj = await Contest.findOne({ status: "CLOSED", endDate: { $gte: previousFirstDate, $lte: previousLastDate } }).sort({ endDate: -1 }).lean().exec();
        if (ContestObj) {
